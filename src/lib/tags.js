...
const NS_CN_MAP = {
  artist: '艺术家', parody: '原作', character: '角色',
  female: '女性', male: '男性', group: '社团',
  series: '系列', language: '语言', reclass: '分类',
  mixed: '混合', other: '其他', cosplayer: 'Cosplayer',
  category: '分类', location: '地点', source: '来源',
  date_added: '添加日期', timestamp: '发布日期', uploader: '发布者'
};
...
export const categorizeTags = (tags) => {
  ...
  tags.forEach((tag) => {
    ...
    if (colonIdx > 0) {
      const ns = tag.slice(0, colonIdx).toLowerCase();
      const value = tag.slice(colonIdx + 1).trim();
      if (!groups[ns]) groups[ns] = [];
      groups[ns].push({ raw: tag, value });
    } else {
      general.push({ raw: tag, value: tag });
    }
  });
  const sorted = [];
  NAMESPACE_ORDER.forEach((ns) => {
    if (groups[ns]) {
      sorted.push({ ns, tags: groups[ns], label: NAMESPACE_LABELS[ns] || ns, color: NAMESPACE_COLORS[ns] || NAMESPACE_COLORS.general });
      delete groups[ns];
    }
  });
...
export const NAMESPACE_COLORS_MAP = NAMESPACE_COLORS;
export const NS_CN_LABELS = NS_CN_MAP;

// ===== Pinyin matching (based on tag-convert.js) =====
const PINYIN_MAP = {
  a:"阿啊呵腌嗄吖锕",e:"额阿俄恶鹅遏鄂厄饿峨扼娥鳄哦蛾噩愕讹锷垩婀鹗萼谔莪腭锇颚呃阏屙苊轭",
  ai:"爱埃艾碍癌哀挨矮隘蔼唉皑哎霭捱暧嫒嗳瑷嗌锿砹",ei:"诶",
  ...
  an:"安案按岸暗鞍氨俺胺铵谙庵黯鹌桉埯犴揞厂广",han:"厂汉韩含旱寒汗涵函喊憾罕焊翰邯撼瀚憨捍酣悍鼾邗颔蚶晗菡旰顸犴焓撖",
  ang:"昂仰盎肮",ao:"奥澳傲熬凹鳌敖遨鏖袄坳翱嗷拗懊岙螯骜獒鏊艹媪廒聱",
  wa:"瓦挖娃洼袜蛙凹哇佤娲呙腽",
  yu:"于与育余预域予遇奥语誉玉鱼雨渔裕愈娱欲吁舆宇羽逾豫郁寓吾狱喻御浴愉禹俞邪榆愚渝尉淤虞峪屿粥驭瑜禺毓钰隅芋熨瘀迂煜昱汩於臾盂聿竽萸妪腴圄谕觎揄龉谀俣馀庾妤瘐鬻欤鹬阈嵛雩鹆圉蜮伛纡窬窳饫蓣狳肀舁蝓燠",
  niu:"牛纽扭钮拗妞忸狃",o:"哦噢喔",
  ...
  cen:"参岑涔",
  san:"三参散伞叁糁馓毵",
  cang:"藏仓苍沧舱臧伧",zang:"藏脏葬赃臧奘驵",
  chen:"称陈沈沉晨琛臣尘辰衬趁忱郴宸谌碜嗔抻榇伧谶龀肜",
  cao:"草操曹槽糙嘈漕螬艚屮",
  ce:"策测册侧厕栅恻",ze:"责则泽择侧咋啧仄箦赜笮舴昃迮帻",
  zhai:"债择齐宅寨侧摘窄斋祭翟砦瘵哜",
  ...
  tan:"谈探坦摊弹炭坛滩贪叹谭潭碳毯瘫檀痰袒坍覃忐忑昙郯澹钽锬",
  ...
  guang:"广光逛潢犷胱咣桄",qiong:"穷琼穹邛茕筇跫蛩銎",
  gao:"高告搞稿膏糕镐皋羔锆杲郜睾诰藁篙缟槁槔",
  hao:"好号毫豪豪耗浩郝皓昊皋蒿壕灏嚎濠蚝貉颢嗥薅嚆",
  ...
  zui:"最罪嘴醉咀蕞觜",
  juan:"卷捐圈眷娟倦绢隽镌涓鹃鄄蠲狷锩桊",suan:"算酸蒜狻",
  yun:"员运云允孕蕴韵酝耘晕匀芸陨纭郧筠恽韫郓氲殒愠昀菀狁",qun:"群裙逡麇",
  ka:"卡喀咖咔咯佧胩",kang:"康抗扛慷炕亢糠伉钪闶",keng:"坑铿吭",
  kao:"考靠烤拷铐栲尻犒",ken:"肯垦恳啃龈裉",
  yin:"因引银印音饮阴隐姻殷淫尹荫吟瘾寅茵圻垠鄞湮蚓氤胤龈窨喑铟洇狺夤廴吲霪茚堙",
  kong:"空控孔恐倥崆箜",ku:"苦库哭酷裤枯窟挎骷堀绔刳喾",
  kua:"跨夸垮挎胯侉",
  kui:"亏奎愧魁馈溃匮葵窥盔逵睽馗聩喟夔篑岿喹揆隗傀暌跬蒉愦悝蝰",
  kuan:"款宽髋",
  kuang:"况矿框狂旷眶匡筐邝圹哐贶夼诳诓纩",
  que:"确却缺雀鹊阙瘸榷炔阕悫",
  kun:"困昆坤捆琨锟鲲醌髡悃阃",kuo:"扩括阔廓蛞",
  la:"拉落垃腊啦辣蜡喇剌旯砬邋瘌",
  lai:"来莱赖睐徕籁涞赉濑癞崃疠铼",
  ...
  mu:"目模木亩幕母牧莫穆姆墓慕牟牡募缪沐暮拇姥钼苜仫毪坶",
  le:"了乐勒肋叻鳓嘞仂泐",
  lei:"类累雷勒泪蕾垒磊擂镭肋羸耒儡嫘缧酹嘞诔檑",
  sui:"随岁虽碎尿隧遂髓穗绥隋邃睢祟濉燧谇眭荽",
  lie:"列烈劣裂猎冽咧趔洌鬣埒捩躐",leng:"冷愣棱楞塄",
  ...
  nong:"农浓弄脓侬哝",shuang:"双爽霜孀泷",
  shu:"术书数属树输束述署熟殊蔬舒疏鼠淑叔暑枢墅俞曙抒竖蜀薯梳戍恕孰沭赎庶漱塾倏澍纾姝菽黍腧秫毹殳疋摅",
  shuai:"率衰帅摔甩蟀",lve:"略掠锊",
  ma:"么马吗摩麻码妈玛嘛骂抹蚂唛蟆犸杩",me:"么麽",
  mai:"买卖麦迈脉埋霾荬劢",
  ...
  mou:"某谋牟缪眸哞鍪蛑侔厶",miu:"缪谬",
  mei:"美没每煤梅媒枚妹眉魅霉昧媚玫酶镁湄寐莓袂楣糜嵋镅浼猸鹛",
  wen:"文问闻稳温纹吻蚊雯紊瘟汶韫刎璺玟阌",
  mie:"灭蔑篾乜咩蠛",
  ming:"明名命鸣铭冥茗溟酩瞑螟暝",
  na:"内南那纳拿哪娜钠呐捺衲镎肭",nei:"内那哪馁",
  nuo:"难诺挪娜糯懦傩喏搦锘",ruo:"若弱偌箬",
  nang:"囊馕囔曩攮",
  nao:"脑闹恼挠瑙淖孬垴铙桡呶硇猱蛲",
  ni:"你尼呢泥疑拟逆倪妮腻匿霓溺旎昵坭铌鲵伲怩睨猊",
  nen:"嫩恁",neng:"能",nin:"您恁",
  niao:"鸟尿溺袅脲茑嬲",
  nie:"摄聂捏涅镍孽捻蘖啮蹑嗫臬镊颞乜陧",
  niang:"娘酿",
  ning:"宁凝拧泞柠咛狞佞聍甯",
  nu:"努怒奴弩驽帑孥胬",nv:"女钕衄恧",
  ru:"入如女乳儒辱汝茹褥孺濡蠕嚅缛溽铷洳薷襦颥蓐",nuan:"暖",nve:"虐疟",
  re:"热若惹喏",
  ou:"区欧偶殴呕禺藕讴鸥瓯沤耦怄",
  pao:"跑炮泡抛刨袍咆疱庖狍匏脬",pou:"剖掊裒",
  pen:"喷盆湓",pie:"瞥撇苤氕丿",
  pin:"品贫聘频拼拚颦姘嫔榀牝",
  se:"色塞瑟涩啬穑铯槭",
  ...
  ran:"然染燃冉苒髯蚺",rang:"让壤攘嚷瓤穰禳",rao:"绕扰饶娆桡荛",
  reng:"仍扔",ri:"日",
  rou:"肉柔揉糅鞣蹂",ruan:"软阮朊",run:"润闰",
  sa:"萨洒撒飒卅仨脎",
  suo:"所些索缩锁莎梭琐嗦唆唢娑蓑羧挲桫嗍睃",
  sai:"思赛塞腮噻鳃",shui:"说水税谁睡氵",
  sang:"桑丧嗓搡颡磉",sen:"森",seng:"僧",
  shai:"筛晒",
  ...
  shua:"刷耍唰",shuan:"栓拴涮闩",shun:"顺瞬舜吮",
  song:"送松宋讼颂耸诵嵩淞怂悚崧凇忪竦菘",
  sou:"艘搜擞嗽嗖叟馊薮飕嗾溲锼螋瞍",
  sun:"损孙笋荪榫隼狲飧",
  teng:"腾疼藤滕誊",tie:"铁贴帖餮萜",
  tu:"土突图途徒涂吐屠兔秃凸荼钍菟堍酴",
  wai:"外歪崴",
  wang:"王望往网忘亡旺汪枉妄惘罔辋魍",weng:"翁嗡瓮蓊蕹",
  zhua:"抓挝爪",
  yang:"样养央阳洋扬杨羊详氧仰秧痒漾疡泱殃恙鸯徉佯怏炀烊鞅蛘",
  xiong:"雄兄熊胸凶匈汹芎",yo:"哟唷",
  yong:"用永拥勇涌泳庸俑踊佣咏雍甬镛臃邕蛹恿慵壅痈鳙墉饔喁",
  za:"杂扎咱砸咋匝咂拶",
  zai:"在再灾载栽仔宰哉崽甾",
  zao:"造早遭枣噪灶燥糟凿躁藻皂澡蚤唣",zei:"贼",zen:"怎谮",
  zeng:"增曾综赠憎锃甑罾缯",zhei:"这",
  zou:"走邹奏揍诹驺陬楱鄹鲰",zhuai:"转拽",
  zun:"尊遵鳟樽撙",dia:"嗲",nou:"耨"
};
...
function buildPinYinIndex() {
  ...
  for (const [py, chars] of Object.entries(PINYIN_MAP)) {
    for (const ch of chars) {
      if (!pinYinIndex[ch]) pinYinIndex[ch] = [];
      pinYinIndex[ch].push(py);
    }
  }
  pinYinReady = true;
}
...
function toPinYin(token) {
  buildPinYinIndex();
  return token.split('').map(ch => pinYinIndex[ch] ? pinYinIndex[ch][0] : ch).join('');
}
...
function pinyinMatch(text, query) {
  ...
  const py = toPinYin(normalized).replace(/\s+/g, '');
  if (py.includes(q)) return true;

  const words = text.split(/\s+/);
  for (const w of words) {
    const initials = toPinYin(w).replace(/\s+/g, '').split('').map((s, i) => {
      const segs = w.split(/\s+/);
      let offset = 0;
      for (const seg of segs) {
        if (i >= offset && i < offset + seg.length) {
          return toPinYin(seg.charAt(i - offset));
        }
        offset += seg.length;
      }
      return s.charAt(0);
...
function parseEhTagDB(rawObj) {
  const optimizedDB = { data: [] };
  const raw = Array.isArray(rawObj.data) ? rawObj.data : (rawObj.data ? Object.entries(rawObj.data).map(([k, v]) => ({ namespace: k, data: v })) : []);

  for (const nsObj of raw) {
    if (!nsObj || !nsObj.data) continue;
    const nsKey = String(nsObj.namespace).toLowerCase();
    const nsData = {};
    for (const [key, val] of Object.entries(nsObj.data)) {
      if (val && val.name) {
        nsData[key.toLowerCase()] = { name: val.name };
      }
    }
    optimizedDB.data.push({ namespace: nsKey, data: nsData });
  }
  return optimizedDB;
}

function loadCache() {
  try { return JSON.parse(window.localStorage.getItem(CACHE_KEY)); } catch { return null; }
}

function saveCache(data) {
  try {
    window.localStorage.setItem(CACHE_KEY, JSON.stringify({ data, time: Date.now() }));
  } catch {}
}
...
function loadDBViaJSONP() {
  if (jsonpLoading) return dbPromise || Promise.resolve();
  jsonpLoading = true;

  dbPromise = new Promise((resolve) => {
    try { delete window.load_ehtagtranslation_db_text; } catch { window.load_ehtagtranslation_db_text = undefined; }

    window.load_ehtagtranslation_db_text = function(rawDb) {
      jsonpLoading = false;
      try {
        const formatted = parseEhTagDB(rawDb);
        translationDB = formatted;
        saveCache(formatted);
        buildSearchIndex();
      } catch (e) { console.error('解析标签翻译库失败:', e); } finally {
        try { delete window.load_ehtagtranslation_db_text; } catch {}
        resolve();
      }
    };
    ...
    script.async = true;
    script.onerror = () => { jsonpLoading = false; resolve(); };
...
export const loadTagDB = () => {
  if (translationDB) return Promise.resolve();

  const cached = loadCache();
  if (cached && cached.data && cached.time && (Date.now() - cached.time <= UPDATE_INTERVAL)) {
    translationDB = cached.data;
    buildSearchIndex();
    return Promise.resolve();
  }

  return loadDBViaJSONP();
};

// ===== Search Index (for tag suggestions) =====
function buildSearchIndex() {
  if (!translationDB || !translationDB.data) return;
  searchIndex = [];
  buildPinYinIndex();

  const seen = new Set();
  for (const nsObj of translationDB.data) {
    const ns = nsObj.namespace.toLowerCase();
    for (const [key, val] of Object.entries(nsObj.data)) {
      const cnName = val.name || '';
      const uid = `${ns}|${key}`;
      if (seen.has(uid)) continue;
      seen.add(uid);
      searchIndex.push({ ns, key, label: cnName, py: toPinYin(cnName) });
    }
  }
  ...
  for (const [key, label] of Object.entries(CATEGORY_MAP)) {
    ...
    if (seen.has(uid)) continue;
    seen.add(uid);
    searchIndex.push({ ns: 'category', key, label, py: toPinYin(label) });
  }
}
...
export const searchTags = (query) => {
  ...
  for (const item of searchIndex) {
    ...
    if (item.key === raw) score = 90;
    else if (item.key.startsWith(raw)) score = 70;
    else if (item.label === raw) score = 60;
    else if (item.label.startsWith(raw)) score = 50;
    else if (item.key.includes(raw)) score = 30;
    else if (item.label.includes(raw)) score = 20;
    else if (!hasChinese && !isExact && pinyinMatch(item.label, raw)) score = 10;
...
export const translateTag = (namespace, text) => {
  if (!text) return text;
  const clean = text.trim().toLowerCase();
  if (namespace === 'category' || namespace === 'reclass') {
    return CATEGORY_MAP[clean] || text;
  }
  ...
  if (!translationDB) return text;
  const nsData = translationDB.data.find(d => d.namespace === namespace);
  if (nsData && nsData.data[clean]) {
    return nsData.data[clean].name;
  }
  return text;
};

let tagDBUpdateTimer = null;

export function startTagDBUpdateTimer() {
  stopTagDBUpdateTimer();
  const doUpdate = () => {
    jsonpLoading = false;
    tagDBUpdateTimer = setTimeout(async () => {
      await loadDBViaJSONP();
      doUpdate();
    }, UPDATE_INTERVAL);
  };
  tagDBUpdateTimer = setTimeout(doUpdate, UPDATE_INTERVAL);
}

export function stopTagDBUpdateTimer() {
  if (tagDBUpdateTimer) { clearTimeout(tagDBUpdateTimer); tagDBUpdateTimer = null; }
}
