  ...
  return <main style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 48px' }}>
    ...
    <section className="glass-panel" style={{ padding: 28, display: 'grid', gap: 18 }}>
      ...
      <div><div style={{ marginBottom: 8 }}>标签</div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>{form.tags.map(tag => <span key={tag} className="btn" title={tag} onClick={async () => { try { await navigator.clipboard.writeText(tag); setStatus(`已复制标签：${tag}`); } catch { setStatus('复制标签失败'); } }} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, cursor: 'copy' }}>{formatMetadataTag(tag, translateTag)}<button type="button" aria-label={`删除 ${tag}`} title="删除标签" onClick={(event) => { event.stopPropagation(); setForm({ ...form, tags: form.tags.filter(item => item !== tag) }); }} style={{ border: 0, background: 'transparent', color: 'var(--text-sub)', cursor: 'pointer', padding: 0, fontSize: 15 }}>×</button></span>)}</div>
      </div>
