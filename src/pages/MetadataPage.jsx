...
export default function MetadataPage({ archiveId }) {
  ...
  const runPlugin = async () => {
    if (!plugin) return;
    setStatus('插件执行中…');
    try { const result = await lrrApi.useMetadataPlugin(archiveId, plugin, pluginArg.trim() || form.title || archive.title || ''); addTags(result?.data?.new_tags || result?.new_tags || ''); setStatus('插件标签已合并，保存后生效。'); }
    catch (error) { setStatus(error.message); }
  };
  ...
  return <main style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 48px' }}>
    ...
    <section className="glass-panel" style={{ padding: 28, display: 'grid', gap: 18 }}>
      ...
      <div><div style={{ marginBottom: 10 }}>标签</div>
