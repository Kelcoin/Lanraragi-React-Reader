...
LRR Modern Reader 是一个面向 [LANraragi](https://github.com/Difegue/LANraragi) 的现代阅读器前端，支持 PWA、标签翻译、分类浏览、阅读历史、智能推荐、归档上传、元数据编辑、EH 评论、重复归档检测和沉浸式阅读。
...
- 上传任务队列：文件和 URL 按顺序独立执行，单项失败不会中断整批，并显示每项处理状态。
...
- 历史页与待看页：集中查看、标签搜索、筛选、批量选择和删除对应记录。
...
- 每个任务显示等待、处理中、成功或失败状态。任务全部结束后会清理 LANraragi 搜索缓存，便于主页显示新归档。
...
- 元数据插件只负责把返回标签合并到当前编辑内容，必须点击“保存元数据”才会写回服务器。
- 页面提供阅读归档、删除归档和返回上一界面操作；删除确认可按本次选择同步移除 EH/EX 收藏夹。
